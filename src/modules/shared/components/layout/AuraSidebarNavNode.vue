    ...
    <q-menu anchor="top right" self="top left" :offset="[8, 0]">
      <q-list dense style="min-width: 220px">
        ...
        <q-item
          ...
          v-close-popup
          v-ripple
          :to="miniLeafRouterTo(leaf)"
          :active="miniLeafManualActive(leaf)"
          ...
          active-class="aura-active"
          class="aura-item"
          @click="(evt) => onMiniLeafClick(leaf, evt)"
        >
...
import {
  matchesSidebarRoute,
  sidebarEntryUsesManualActive,
} from 'src/modules/shared/utils/sidebarRouteMatch.js'
...
const leafUsesManualActive = computed(() => sidebarEntryUsesManualActive(normalizedEntry.value))

const leafRouterTo = computed(() => {
  if (normalizedEntry.value.disabled || !normalizedEntry.value.to) return undefined
  if (leafUsesManualActive.value) return undefined
  return normalizedEntry.value.to
})

const leafManualActive = computed(() => {
  if (!leafUsesManualActive.value || normalizedEntry.value.disabled) return undefined
  return matchesSidebarRoute(route, normalizedEntry.value.to, router)
})
...
function onLeafClick() {
  const to = normalizedEntry.value.to
  if (leafUsesManualActive.value && to && !normalizedEntry.value.disabled) {
    router.push(to)
  }
  onNavigate()
}

function miniLeafRouterTo(leaf) {
  if (leaf.disabled || leaf.href || !leaf.to) return undefined
  if (sidebarEntryUsesManualActive(leaf)) return undefined
  return leaf.to
}

function miniLeafManualActive(leaf) {
  if (!sidebarEntryUsesManualActive(leaf) || leaf.disabled || leaf.href) return undefined
  return matchesSidebarRoute(route, leaf.to, router)
}

function onMiniLeafClick(leaf, evt) {
  if (sidebarEntryUsesManualActive(leaf) && leaf.to && !leaf.disabled && !leaf.href) {
    evt?.preventDefault?.()
    router.push(leaf.to)
  }
  onNavigate()
