<template>
  ...
  >
    <div class="card-content">
      <q-icon name="edit_document" size="3rem" class="text-primary q-mb-md" />
      <div class="text-h4 text-primary font-weight-bold">
        {{$t('LiquidationPF.saisieCardTitle')}}
      </div>
  ...
  >
    ...
    <div class="header-bar">
      ...
      <div class="header-title">
        <span class="text-h6 text-primary font-weight-bold">{{$t('LiquidationPF.headerTitle')}}</span>
      </div>
    </div>
            ...
            <div class="sidebar-content">
              <q-list>
                ...
                >
                  ...
                  <q-item-section>
                    <q-item-label class="text-primary font-weight-bold">{{$t('LiquidationPF.sidebar.' + getSidebarKey(item.label))}}</q-item-label>
                  </q-item-section>
                </q-item>
              </q-list>
            </div>
...
<script setup>
import { ref } from 'vue'
import Aperiodique from './Aperiodique.vue'
import SaisieReprises from './SaisieReprises.vue'
import AllocationsFamiliales from './AllocationsFamiliales.vue'
import PeriodeActive from './PeriodeActive.vue'
import PieceMaintienDroit from './PieceMaintienDroit.vue'
...
const items = [
  { label: 'Apériodique', icon: 'event_repeat', component: Aperiodique },
  { label: 'Saisie des reprises', icon: 'edit_note', component: SaisieReprises },
  { label: 'Allocations Familiales', icon: 'family_restroom', component: AllocationsFamiliales },
  { label: 'Période active', icon: 'calendar_month', component: PeriodeActive },
  { label: 'Pièce de maintien de droit', icon: 'description', component: PieceMaintienDroit },
  { label: 'Statistiques', icon: 'bar_chart', component: Statistiques },
]
...
// Ajout d'une fonction utilitaire pour faire correspondre les labels aux clés de traduction de la sidebar
function getSidebarKey(label) {
  switch (label) {
    case 'Apériodique':
      return 'aperiodique';
    case 'Saisie des reprises':
      return 'reprises';
    case 'Allocations Familiales':
      return 'allocations';
    case 'Période active':
      return 'periodeActive';
    case 'Pièce de maintien de droit':
      return 'pieceMaintien';
    case 'Statistiques':
      return 'statistiques';
    default:
      return label;
